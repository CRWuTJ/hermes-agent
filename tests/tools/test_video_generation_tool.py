...
def test_text_to_video_submits_default_fal_endpoint_and_saves_mp4(monkeypatch, tmp_path):
    ...
    result = json.loads(
        video_generation_tool.video_generate_tool(
    ...
    assert result["video"] == "https://v3.fal.media/files/lion/render_output.mp4"
    ...
    assert captured["model"] == "fal-ai/kling-video/v2.1/master/text-to-video"
...
def test_image_to_video_requires_image_url_and_submits_image_endpoint(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "_submit_fal_request",
        lambda model, arguments: captured.update({"model": model, "arguments": arguments}) or FakeHandle(),
    )
    monkeypatch.setattr(
        video_generation_tool.urllib.request,
        "urlopen",
        lambda request, timeout=None: _FakeDownloadResponse(b"image-to-video"),
    )
